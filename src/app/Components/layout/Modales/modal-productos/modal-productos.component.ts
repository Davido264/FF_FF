...
guardarEditar_Producto(){
  ...
  if(this.datosProducto == null){
     ...
     next:(data) =>{
       ...
       this._utilidadServicio.mostrarAlerta("No se pudo regitrar el producto","Error")
    ...
    this._productoServicio.editar(_producto).subscribe({
      next:(data) =>{
        if(data.status){
         this._utilidadServicio.mostrarAlerta("El producto fue actualizado correctamente","Exito");
         this.modalActual.close("true")
        }else 
        this._utilidadServicio.mostrarAlerta("No se pudo actualizar al producto","Error")
